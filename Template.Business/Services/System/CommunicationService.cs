    ...
    public class CommunicationService : ICommunicationService
    {
        ...
        public async Task SendConfirmEmailAsync(string to, string template_name, string config_name = "")
        {
            try
            {
                ...
                var configs = await databaseService.GetAsync<TblEmailConfig>(x => x.Name == (config_name ?? EmailConfig.Default));

                if (configs == null)
                {
                    logger.LogWarning($"Failed to get email configuration: {config_name ?? EmailConfig.Default}");
